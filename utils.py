        ...
        try:
            ...
            if num_businessinfoitem:
                ...
                if alls.get("register_num", "") == "":
                    try:
                        self.cursor.execute(sql3, (item["status"], item["company_type"], item["establish_date"], item["expire_date"],item["register_money"], item["register_authority"], item["business_scope"],item["register_address"], item["legal_representative"], item["register_num"],item["company_website"], item["industry"], item["company_name"]))
                        # return num_businessinfoitem
    ...
    def insertEs(self,item):
        if item["register_address"]:
            ...
            try:
                ...
                erro = "插入es异常,异常信息{}".format(e)
                return erro
                # self.loging.info("插入es异常,异常信息".format(e))
